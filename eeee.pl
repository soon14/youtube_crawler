...
my ($version) = ('$Revision: 1.908 $' =~ m/\s(\d[.\d]+)\s/s);
...
sub bbb(){
  my $cipher="player-en_US-vfl5-0t5t/base";
  my $sig = "22FF7351C5942530575B8FCBF6323772170F29916.EFB26561D9E0E51B13E8E2BB482A5FEA49E735644";
  my $id = $cipher . " " . length ($sig);
  my $got = decipher_sig ($id, $cipher, $sig);
  print STDERR "$got\n";  
}
...
sub load_youtube_formats($$) {
  ...
  if ($err) {
    if ($verbose <= 0 && $err =~ m/\bprivate\b/si) {
